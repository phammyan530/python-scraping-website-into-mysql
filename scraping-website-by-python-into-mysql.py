...
for url in tqdm(all_page_link("https://www.imdb.com/list/ls068082370/")):
    soup = BeautifulSoup(requests.get(url).text, "html.parser")
    for itemmovie in soup.find_all(class_='lister-item mode-detail'):
        link = itemmovie.find(class_='lister-item-content')
        ID = int(link.find('span', {"class": "lister-item-index unbold text-primary"}).text[:-1])
        Movie = link.find('a').text
        Descrt = link.find('p', {"class": ""}).text
        # get image link and repair
        img_div = itemmovie.find(class_='lister-item-image')
        img_href = img_div.find('img').attrs["loadlate"]
        img_array = img_href.split("@._V1")
        img = img_array[0] + "@._V1_.jpg"
        # end: get image link and repair
        ...
        movie_info = [ID, Movie, Year, Run_Time, Genre, Director, Actors, Descrt, img]
...
insert_movie_query = """
INSERT INTO movies
(id, movie, year, run_time, genre, director, actors, descrt, img)
